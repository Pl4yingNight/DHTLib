...
#define dht_DDR_in() DDR(DHT_PORT) &= ~(1<<DHT_PIN)
#define dht_DDR_out() DDR(DHT_PORT) |= (1<<DHT_PIN)
#define dht_PIN_low() DHT_PORT &= ~(1<<DHT_PIN)
#define dht_PIN_high() DHT_PORT |= (1<<DHT_PIN)
...
extern uint8_t dht_rh_high(){
	return I_RH;
}
extern uint8_t dht_rh_low(){
	return D_RH;
}
extern uint8_t dht_temp_high(){
	return I_Temp;
}
extern uint8_t dht_temp_low(){
	return D_Temp;
}